...
def _chunks_to_citations(filtered_chunks):
    ...
    for chunk in sorted_chunks:
        try:
            ...
            # Add page number to the set (automatically handles duplicates)
            pdf_page_groups[source_pdf]["pages"].add(page_no)
            pdf_page_groups[source_pdf]["chunks"].append(chunk)
            
            # Track the best score for this PDF
            similarity_val = float(chunk.get("similarity_score", 0) or 0)
            combined_score = similarity_val + float(chunk.get("tfidf_score", 0) or 0)
            if combined_score > pdf_page_groups[source_pdf]["best_score"]:
                pdf_page_groups[source_pdf]["best_score"] = combined_score
            if similarity_val > pdf_page_groups[source_pdf]["best_similarity"]:
                pdf_page_groups[source_pdf]["best_similarity"] = similarity_val
                
        except Exception:
            continue
    
    # Convert grouped data to citations (limit to top 3 PDFs)
    # Sort primarily by highest similarity, tiebreaker by combined score
    sorted_pdfs = sorted(
    ...
    for source_pdf, data in sorted_pdfs:
        ...
        # Sort pages by their best score (highest first)
        sorted_pages_data = sorted(page_scores.items(), key=lambda x: x[1], reverse=True)
        sorted_pages = [page for page, score in sorted_pages_data]  # Extract just the page numbers
        
        # Debug: Print what we're processing
        print(f"[SEARCH] Processing PDF: {source_pdf}, Pages (by relevance): {sorted_pages}")
        
        # Format page numbers showing most relevant first
        if len(sorted_pages) == 1:
            page_display = f"Page: {sorted_pages[0]}"
        else:
            # Show most relevant pages first
            page_display = f"Pages: {', '.join(map(str, sorted_pages))}"
        
        print(f"[PDF] Formatted page display: {page_display}")
        ...
        citation = {
            ...
            "page_display": page_display,  # New field for formatted page numbers
            "page_numbers": sorted_pages,   # Array of all page numbers
            "similarity_score": float(best_chunk.get("similarity_score", 0.0)),
            "tfidf_score": float(best_chunk.get("tfidf_score", 0.0)),
            "combined_score": float(best_chunk.get("similarity_score", 0.0) + best_chunk.get("tfidf_score", 0.0)),
...
def get_answer(query, conversation_id: str | None = None, pdf_context: str = None):
    import time
    ...
    if not in_memory_embeddings:
        ...
        try:
            ...
            if count > 0:
                ...
                return {
                    "answer": "No documents are currently available for querying. Please ensure documents are processed.",
                    "citations": [],
                    "follow_up_questions": [
                        "What documents are available?",
                        "How can I process documents?",
    ...
    if pdf_context:
        ...
        pdf_filtered_chunks = {}
        pdf_filtered_embeddings = {}
        
        for chunk_id, chunk_data in in_memory_chunks.items():
            source_pdf = chunk_data.get('source_pdf', '')
            
            # Exact match only - to avoid returning wrong documents  
            matches = source_pdf.lower() == pdf_context.lower()
            
            if matches:
                pdf_filtered_chunks[chunk_id] = chunk_data
                if chunk_id in in_memory_embeddings:
                    pdf_filtered_embeddings[chunk_id] = in_memory_embeddings[chunk_id]
        
        print(f"[PDF_FILTER] Found {len(pdf_filtered_chunks)} chunks for PDF: {pdf_context}")
        
        if not pdf_filtered_chunks:
                return {
                    "answer": f"No relevant information found in {pdf_context} for your query. The document may not contain content related to your question.",
                    "citations": [],
                    "follow_up_questions": [
                        f"What information is available in {pdf_context}?",
    ...
    if not filtered_chunks:
        return {"answer": "No relevant information found.", "citations": [], "follow_up_questions": []}
    ...
    citations = _chunks_to_citations(filtered_chunks)
    ...
    for i, citation in enumerate(citations):
        print(f"  {i+1}. {citation.get('source_pdf', 'Unknown')} - {citation.get('page_display', 'No pages')}")
    
    # Generate follow-up questions
    followup_start = time.perf_counter()
    follow_up_questions = generate_follow_up_questions(answer, query)
    ...
    return {"answer": answer, "citations": citations, "follow_up_questions": follow_up_questions}
